...
public class Barcode {
    ...
    public static String createBarCode(String savePath, String jbarCode,
        ...
        if (jbarCode.length() != 13) {
            System.out.println("barcode is required len = 13");
        }
        File dir = new File(savePath);
        if (!dir.exists()) {
            if (!dir.isDirectory()) {
                System.out.println(savePath + " is not a directory, require a directory");
                return null;
            }
            dir.mkdir();
            System.out.println("create a directory " + savePath);
        } else {
